...
data <- list(y=y)
inits <- list(phi=rep(0.6, k -1) , p=rep(0.9, k), x=x_init)
...
## define as data also latent states
...
##
# round(apply(origMCMCSamples,2,mean), 3)
# round(apply(origMCMCSamples,2,sd), 3)
...
library(R2ucare) ## for function marray
