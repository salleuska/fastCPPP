...
# 			cpppBootApprox <- numeric(bootIters)
# 			cpppBootApproxQuantile <- numeric(bootIters)
...
# 				cpppBootApprox[j] <- mean(kappaBoot <= M[m]* obsPPP)

# 				kappaBootProbs <- pnorm(M[m]*obsPPP, mean = meanVec, sd = sqrt(varVec))				
# 				cpppBootApproxQuantile[j] <- mean(kappaBootProbs)
...
# 			tmpNormal[k] <- var(cpppBootApprox)
...
# dimnamesvarianceMBB) <- list(compCost, M)
