...
# args <- list()
# args$dirExample <- "sec6_examples/newcomb/badMixing/"
...
dfPlot$lower <- ifelse(dfPlot$cpppEst - 1.96*dfPlot$value > 0, dfPlot$cpppEst - 1.96*dfPlot$value, 0)
dfPlot$upper <- ifelse(dfPlot$cpppEst +1.96* dfPlot$value < 1 , dfPlot$cpppEst + 1.96*dfPlot$value, 1)
...
for(i in 1:length(levels(dfPlot$ComputationalCost))){
	...
	plotList[[i]] <- ggplot(subsetDfPlot,
		...
		annotate("rect", xmin = -Inf, xmax = Inf, ymin = trueCPPP - 1.96*sqrt(trueCPPPVar),
		 ymax = trueCPPP +1.96*sqrt(trueCPPPVar), fill = "gray80", alpha = .5) + 
		...
		geom_text(aes(label = paste0("r=", Replicates), y = maxVal + 0.05),
...
ggsave(plotToSave, file = paste0("figures/", basename(dirExample), "_varComparison2.png"), 
...
# Rscript 9_testplot.R --dirExample="sec6_examples/newcomb" --plotTitle="Newcomb example"
# Rscript 9_testplot.R --dirExample="sec6_examples/newcombBadMixing" --plotTitle="Newcomb example - bad mixing"
# Rscript 9_testplot.R --dirExample="sec6_examples/dipperTT" --plotTitle="dipperTT example"
# Rscript 9_testplot.R --dirExample="sec6_examples/dipperCC" --plotTitle="dipperCC example"
# Rscript 9_testplot.R --dirExample="sec6_examples/capRecapSimulated" --plotTitle="capRecapSimulated example"

########################################
## plotting for ess arond 200
# q	
